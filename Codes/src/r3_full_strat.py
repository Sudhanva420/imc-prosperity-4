...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for sym in ["VELVETFRUIT_EXTRACT", "HYDROGEL_PACK"]:
            if sym in state.order_depths and sym == "HYDROGEL_PACK":
                result[sym] = self.delta_one_hyd(sym, state.order_depths[sym], state.position.get(sym, 0))
            else:
                result[sym] = self.delta_one_vfe(sym, state.order_depths[sym], state.position.get(sym, 0))
